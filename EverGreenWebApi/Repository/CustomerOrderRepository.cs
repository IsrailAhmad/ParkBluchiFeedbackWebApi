    ...
    public class CustomerOrderRepository : ICustomerOrderRepository
    {
        ...
        public CustomerOrderModel OrderPlaced(CustomerOrderModel model)
        {
            ...
            using (storefeedback_androidEntities context = new storefeedback_androidEntities())
            {
                ...
                var advancepaid = 0.0;
                if (model.AdvancePrice > 0)
                {
                    advancepaid = (double)model.AdvancePrice;
                    var advance = context.advancemasters.Where(x => x.OrderNumber == strSequnceNumber && x.AdvancePrice == model.AdvancePrice).Select(s => s.AdvancePrice).FirstOrDefault();
                    if (advance == null)
                    {
                        advancemaster ad = new advancemaster();
                        ad.StoreId = model.StoreId;
                        ad.OrderNumber = strSequnceNumber;
                        ad.AdvancePrice = model.AdvancePrice;
                        context.advancemasters.Add(ad);
                    }
                ...
                //{
                    ...
                    //    advancepaid = 0.0;
                    //}
                    ////var advance = context.advancemasters.Where(x => x.OrderNumber == strSequnceNumber).Select(s => s.AdvancePrice).ToList();
               ...
               // }
                ...
                if (order.AdvancePrice > 0)
                {
                    SendSMS(order, 1);
                    SendEmail(order, categorylist, extraproductList, 1);
                }
                return order;
            }
        }
        public CustomerOrderModel EventOrderUpdate(CustomerOrderModel model)
        {
            ...
            using (storefeedback_androidEntities context = new storefeedback_androidEntities())
            {
                ...
                if (o != null)
                {
                    ...
                    if (model.AdvancePrice > 0)
                    {
                        ...
                        if (advance == null)
                        {
                            advancemaster ad = new advancemaster();
                            ad.StoreId = model.StoreId;
                            ad.OrderNumber = model.OrderNumber;
                            ad.AdvancePrice = model.AdvancePrice;
                            context.advancemasters.Add(ad);
                        }
                        else
                        {
                             var adv = context.advancemasters.Where(x => x.OrderNumber == model.OrderNumber);
                            if (adv != null)
                            {

                                foreach (var ad in adv)
                                {
                                    ad.AdvancePrice = (decimal)model.AdvancePrice;
                                }
                                context.SaveChanges();
                            }
                        ...
                        //advancepaid = (double)model.AdvancePrice;
                        //var advance = context.advancemasters.Where(x => x.OrderNumber == model.OrderNumber);
                        //if (advance == null)
                        //{
                        //    advancemaster ad = new advancemaster();
                        //    ad.StoreId = model.StoreId;
                ...
                {
                    SendSMS(order, 2);
                    SendEmail(order, categorylist, extraproductList, 2);
                }
                data = order;
            }
            return data;
        ...
        public ResponseStatus EventOrderDelete(string strOrderNumber, int customerid, int storeid)
        {
            ...
            using (storefeedback_androidEntities context = new storefeedback_androidEntities())
            {
                ...
                if (ordernumber != null)
                {
                    string CustExtraProducts = "";
                    extraproductList = (from ep in context.customerextraproductmasters
                                        where ep.CustomerId == customerid && ep.OrderNumber == strOrderNumber
                                        select new ProductModel()
                                        {
                                            ProductName = ep.ProductName
                                        }).ToList();
                    ...
                    var data = (from x in context.customerordermasters
                                join c in context.customermasters on x.CustomerId equals c.CustomerId
                                join ev in context.customereventmasters on x.CustomerId equals ev.CustomerId into j1
                                from j2 in j1.DefaultIfEmpty()
                                join m in context.menumasters on x.MenuId equals m.MenuId into j3
                                from j4 in j3.DefaultIfEmpty()
                                where x.CustomerId == customerid && x.StoreId == storeid && x.OrderNumber == strOrderNumber
                                orderby x.OrderId descending
                                select new CustomerOrderModel()
                                {
                                    OrderId = x.OrderId,
                                    ...
                                    Extra = (from e in context.customerextramasters
                                             join ex in context.extrachargemasters on e.ExtraId equals ex.ExtraId into j1
                                             from j2 in j1.DefaultIfEmpty()
                                             where e.OrderNumber == strOrderNumber && e.CustomerId == customerid
                                             orderby e.ExtraId ascending
                                             select new ExtraChargesModel()
                                             {
                                                 ExtraId = (int)e.ExtraId,
                                    ...
                                    Category = (from c in context.customercategorymasters
                                                where c.OrderNumber == strOrderNumber
                                                orderby c.CategoryId ascending
                                                select new CustomerCategoryModel()
                                                {
                                                    CategoryId = (int)c.CategoryId,
                                                    Product = (from p in context.customerproductmasters
                                                               join kt in context.productmasters on p.ProductId equals kt.ProductId into r1
                                                               from r2 in r1.DefaultIfEmpty()
                                                               where p.OrderNumber == strOrderNumber && p.CategoryId == c.CategoryId
                                                               orderby p.ProductId ascending
                                                               select new CustomerProductModel()
                                                               {
                                                                   ProductId = (int)p.ProductId,
                    ...
                    categorylist = data.Category.ToList();
                    ...
                    if (result > 0)
                    {
                        ...
                        {
                            SendSMS(data, 4);
                            SendEmail(data, categorylist, extraproductList, 4);
                        }
        ...
        public ResponseStatus EventOrderCancel(string strOrderNumber, int customerid, int storeid)
        {
            ResponseStatus response = new ResponseStatus();
            using (storefeedback_androidEntities context = new storefeedback_androidEntities())
            {
                ...
                if (ordernumber != null)
                {
                    ...
                    //context.customerextramasters.RemoveRange(context.customerextramasters.Where(e => e.OrderNumber == strOrderNumber));
                    //context.customerordermasters.RemoveRange(context.customerordermasters.Where(o => o.CustomerId == customerid && o.OrderNumber == strOrderNumber));
                    var cancelOrder = context.customerordermasters.Where(o => o.CustomerId == customerid && o.OrderNumber == strOrderNumber);
                    foreach (var o in cancelOrder)
                    {
                        o.OrderStatusId = 3;
                    }
                    var result = context.SaveChanges();
                    if (result > 0)
                    {
                        ...
                        //{
                        SendSMS(data, 5);
                        SendEmail(data, categorylist, extraproductList, 5);
                        //}
                        response.isSuccess = true;
                        response.serverResponseTime = System.DateTime.Now;
                    }
        ...
        public CustomerOrderModel GetAllEventOrderByOrderNumber(string OderNumber)
        {
            ...
            using (storefeedback_androidEntities context = new storefeedback_androidEntities())
            {
                ...
                order = (from x in context.customerordermasters
                         ...
                         from j4 in j3.DefaultIfEmpty()
                         join os in context.orderstatusmasters on x.OrderStatusId equals os.OrderStatusId into j5
                         ...
                         select new CustomerOrderModel()
                         {
                             ...
                             AdvancePrice = x.AdvancePrice != null ? (decimal)x.AdvancePrice : (decimal)0.0,
                             Total = (decimal)x.Total,
                             GrandTotal = (decimal)x.GrandTotal,
                             NetPrice = (decimal)x.NetPrice,
                             DiscountPercent = (decimal)x.DiscountPercent,
        ...
        public CustomerOrderModel PrintReceipt(PrintReceiptModel model)
        {
            ...
            {
                SendSMS(result, 3);
                SendEmail(result, categorylist, extraproductList, 3);
            }
            return result;
        }
        public void SendSMS(CustomerOrderModel order, int OrderStatusId)
        {
            string header = "";
            ...
            if (OrderStatusId == 1)
            {
                header = "Your Party Booking Request Created with Order No:";
            }
            if (OrderStatusId == 2)
            {
                header = "Your Party Booking Request Updated with Order No:";
            }
            if (OrderStatusId == 3)
            {
                header = "Your Party Booking Request confirmed with Order No:";
            }
            if (OrderStatusId == 4)
            {
                header = "Your Party Booking Request Deleted with Order No:";
            }
            if (OrderStatusId == 5)
            {
                header = "Your Party Booking Request Cancaled with Order No:";
            }
            ...
            using (storefeedback_androidEntities context = new storefeedback_androidEntities())
            {
                ...
                receiverno += order.CustomerPhoneNo;
            }
        ...
        public void SendEmail(CustomerOrderModel order, List<CustomerCategoryModel> categorylist, List<ProductModel> extraproductList, int OrderStatusId)
        {
            ...
            if (OrderStatusId == 1)
            {
                subject = "Your Party Booking Request Created with Order No: " + order.OrderNumber + "";
            }
            if (OrderStatusId == 2)
            {
                subject = "Your Party Booking Request Updated with Order No: " + order.OrderNumber + "";
            }
            if (OrderStatusId == 3)
            {
                subject = "Your Party Booking Request confirmed with Order No: " + order.OrderNumber + "";
            }
            if (OrderStatusId == 4)
            {
                subject = "Your Party Booking Request Deleted with Order No: " + order.OrderNumber + "";
            }
            if (OrderStatusId == 5)
            {
                subject = "Your Party Booking Request Canceled with Order No: " + order.OrderNumber + "";
            }
